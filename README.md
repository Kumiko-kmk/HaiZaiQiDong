...
- **可编辑自定义画布**：使用画笔、橡皮擦、撤回、前进和垃圾桶修改图案，也能把已有自定义预设载入画布继续编辑并原位保存。
- **JSON / SVG 导入**：支持常见矢量图元、曲线、圆弧和变换；粗填充轮廓会提取笔画中心线，避免生成双层空心线条。
...
- **今日鸡煲笑话**：根据系统日期每天固定展示一则笑话，无需滚动即可阅读完整内容。
...
| 今日鸡煲笑话 | — | 按本机系统日期每天固定展示一则笑话；标题与正文完整显示在静态弹窗中，不需要滚动或缩放。 |
| 关于 | — | 查看程序用途和基础操作提示。 |
...
后端还提供 JSON 与常见 SVG 图元的导入能力，包括 `path`、`line`、`polyline`、`polygon`、`circle`、`ellipse` 和 `rect`，并支持贝塞尔曲线、圆弧以及 `matrix`、`translate`、`scale` 变换。明确使用纯色填充表示粗线条的封闭轮廓会先提取中心线；文字、位图、滤镜、蒙版和 CSS 视觉效果不会转换成笔画。若要批量制作预设，可参考 `presets/data` 中的现有文件和 `tools` 目录中的轨迹辅助工具。
...
如果《杀戮尖塔 2》以管理员身份运行，普通权限的工具会被 Windows 的输入权限隔离。当前版本启动时会自动显示 UAC 提示，请选择“是”让工具以管理员权限继续运行；若拒绝授权，程序不会进入主界面。已经允许 UAC 但仍无法落点时，可检查 `%APPDATA%\HaiZaiQiDong\logs\runtime.log` 中的输入注入错误。
